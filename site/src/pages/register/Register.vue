    ...
    <span slot="main">
      <span v-if="registerTab === 0">
        <span class="center-align">
          <h5>Cadastro - Dados para login</h5>
        </span>
      ...
      switch (this.registerTab) {
        case 0:
          ...
          if (this.user.password.trim() === "") {
            error = this.concErro(error);
            error = error + "Campo senha não informado!";
          }
          if (this.user.password_confirmation !== this.user.password.trim()) {
            error = this.concErro(error);
